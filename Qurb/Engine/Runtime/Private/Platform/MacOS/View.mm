...
- (instancetype)initWithWindow:(Window*)initWindow frame:(NSRect)initFrame
{
    ...
    if (self != nil)
    {
        ...
        trackingArea = nil;
        markedText   = [NSMutableAttributedString new];
...
- (BOOL)isOpaque
{
    // return [baseWindow->nsWindow() isOpaque];
    return YES;
}
...
- (void)mouseMoved:(NSEvent*)event
{
    ...
    // Input::setMousePosition(Vector2f(position.x, position.y));
}
...
{
    // const NSRect frame = [[baseWindow->nsWindow() contentView] frame];
